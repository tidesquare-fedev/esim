...
const ProductList: React.FC<ProductListProps> = ({ selectedCountries, products, onProductSelect, onBack }) => {
    const { commonProducts, individualProductsByCountry } = useMemo(() => {
        ...
        const commonProducts = products.filter(p =>
            selectedCountryIds.every(id => p.supportedCountries.includes(id))
        );
        const commonProductIds = new Set(commonProducts.map(p => p.id));

        const individualProductsByCountry = selectedCountries.map(country => {
            const countrySpecificProducts = products.filter(p =>
                p.supportedCountries.includes(country.id) && !commonProductIds.has(p.id)
            );
            return {
        ...
        <div className="w-full max-w-[750px] mx-auto px-4">
            ...
            {hasProducts ? (
                <div className="space-y-8">
                    ...
                    {individualProductsByCountry.map(group => (
                        <section key={group.countryName}>
                             <h2 className="text-xl font-bold mb-4 text-gray-800">
                                ✈️ {group.countryName} 추천 상품
                            </h2>
                            <div className="space-y-4">
                                {group.products.map(product => (
                                    <ProductCard key={`${group.countryName}-${product.id}`} product={product} onProductSelect={onProductSelect} />
                                ))}
                            </div>
                        </section>
                    ))}
                ...
                <div className="text-center py-20">
                    ...
                    <p className="text-gray-500 mt-2">선택하신 국가 조합에 맞는 상품이 없어요.<br />국가를 다시 선택해주세요.</p>
                    <button
                        onClick={onBack}
